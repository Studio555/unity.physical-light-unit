...
namespace UnityEngine.Rendering.Universal
{
	...
	public partial class AdditionalLightData : MonoBehaviour, ISerializationCallbackReceiver, IAdditionalData
	{
		...
		/// </summary>
		public const float
			k_DefaultAreaLightIntensity = 200.0f; // Light default to 200 lumen to better match point light
		...
		public LightUnit lightUnit
		{
			...
			set
			{
				...
				if (!IsValidLightUnitForType(legacyLight.type, value))
				{
					...
					Debug.LogError(
						$"Set Light Unit '{value}' to a {GetLightTypeName()} is not allowed, only {supportedTypes} are supported.");
					return;
				}
		...
		[SerializeField] float m_ShadowFadeDistance = 10000.0f;
...
#pragma warning restore 0414
		...
		[System.NonSerialized] TimelineWorkaround timelineWorkaround = new TimelineWorkaround();
		...
		[System.NonSerialized] bool m_Animated;
		...
		public static void InitDefaultHDAdditionalLightData(AdditionalLightData lightData)
		{
			...
			switch (lightData.legacyLight.type)
			{
				case LightType.Directional:
					...
					lightData.intensity =
						k_DefaultDirectionalLightIntensity / Mathf.PI *
						100000.0f; // Change back to just k_DefaultDirectionalLightIntensity on 11.0.0 (can't change constant as it's a breaking change)
					break;
		...
		{
			// Apply hardcoded interior boost for point and spot lights
			float finalIntensity = intensity;
			if (legacyLight.type == LightType.Point || legacyLight.type == LightType.Spot)
			{
				finalIntensity *= HARDCODED_INTERIOR_BOOST;
			}
				...
				case LightType.Spot:
					...
					else // lumen
					{
						if (enableSpotReflector)
						{
							...
							// {
							legacyLight.intensity = LightUtils.ConvertSpotLightLumenToCandela(finalIntensity,
								legacyLight.spotAngle * Mathf.Deg2Rad, true);
							// }
 ...
 LightUtils.ConvertAreaLightLumenToLuminance(legacyLight.type, intensity, legacyLight.areaSize.x, legacyLight.areaSize.y);
#endif
			}
			...
			// Apply hardcoded exposure compensation (EV100 = 13 for sunlight)
			float exposureCompensation = ColorUtils.ConvertEV100ToExposure(HARDCODED_EXPOSURE_EV100);
			legacyLight.intensity *= exposureCompensation;
		...
		internal bool IsOverlapping()
		{
			...
			bool isDirectUsingBakedOcclusion = baking.mixedLightingMode == MixedLightingMode.Shadowmask ||
